...
function renderChart(chartData) {
    ...
    if (chartType === 'bar') {
        ...
        if (canvas.tagName !== 'CANVAS') {
            ...
            parent.innerHTML = '<canvas id="rankingChart"></canvas>';
        ...
        const items = chartData.items.slice(0, 20); // Limit pie chart to 20 items
        const values = items.map(item => {
            const key = chartData.sortKey;
            return item[key] ?? 0;
        });
        
        const labels = items.map(item => {
            let label = `#${item._staticRank} ${item.name}`;
            if (item.author && !chartData.isAuthor) {
                label += ` (${item.author})`;
            }
            return label;
        });
        ...
        const backgroundColors = items.map((_, i) => {
            ...
            return colors[i % colors.length] + '99'; // Add transparency
        });
